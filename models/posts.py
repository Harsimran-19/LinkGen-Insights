...
from src.embedding import EmbeddingModelSingleton
...
class RawPost(BaseModel):
    ...
    def from_source(cls, k_v: Tuple[str, dict]) -> "RawPost":
        ...
        return cls(
            post_id=k,
            post_owner=v.get("name", "Unknown"),  # Default to "Unknown" if "name" key is missing
            source=v.get("source", "Unknown"),    # Default to "Unknown" if "source" key is missing
            text=v.get("text", ""),               # Default to empty string if "text" key is missing
            image=v.get("image", None)            # Default to None if "image" key is missing
        )
...
class CleanedPost(BaseModel):
    ...
    def from_raw_post(cls, raw_post: RawPost) -> "CleanedPost":
        ...
        return cls(
            ...
            text=cleaned_text,
            post_owner=raw_post.post_owner,
    ...
    def from_cleaned_post(
        ...
        return [
            cls(
                ...
                post_owner=cleaned_post.post_owner,
...
class EmbeddedChunkedPost(BaseModel):
    ...
    def from_chunked_post(
        ...
        return cls(
            ...
            post_owner=chunked_post.post_owner,
            source=chunked_post.source,
    ...
    @classmethod
    def from_retrieved_point(
        cls, point: Union[ScoredPoint, Record]
    ) -> "EmbeddedChunkedPost":
        return cls(
            post_id=point.payload["post_id"],
            post_owner=point.payload['post_owner'],
